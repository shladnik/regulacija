...
{
  assert(config_range(adr, len) || writeallow_range(adr, len));
...
USED void flash_write(uint8_t * buf, uintptr_t adr, uintptr_t len)
{
  ...
  if (start_floor) {
    ...
    flash_write_block(buf, adr, len_s, check);
    buf += len_s;
    adr += len_s;
    len -= len_s;
  }
  ...
  while (adr < end_floor) {
    flash_write_block(buf, adr, SPM_PAGESIZE, check);
    buf += SPM_PAGESIZE;
    adr += SPM_PAGESIZE;
    len -= SPM_PAGESIZE;
  }
  ...
  if (len) {
    flash_write_block(buf, adr, len, check);
    buf += len;
    adr += len;
    len -= len;
  }
